...
class BirkhoffPolytope(Manifold):
    ...
    def __init__(self, n, m, k=1, p=None, q=None, max_iter=100, tol=1e-5, eps=1e-12):
        super().__init__()
        ...
        self._p = p
        self._q = q
        self._size = torch.Size((self._n, self._m))
    ...
    def random_naive(self, *size, dtype=None) -> torch.Tensor:
        """
    ...
    def origin(self, *size, dtype=None, seed=42) -> torch.Tensor:
        """
...
def proj_tangent(x, u):
    ...
    alpha = torch.cat(
        [torch.ones(batch_size, 1, 1, dtype=x.dtype).to(x.device), zeta[:, 0 : n - 1]], dim=1
    )
    beta = zeta[:, n - 1 : 2 * n - 1]
