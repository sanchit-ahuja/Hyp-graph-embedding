...
class ManifoldShapeFactory(object):
    ...
    @staticmethod
    def create_manifold_parameter(manifold, shape, transpose=False, p_manifold=None):
        if manifold not in ManifoldShapeFactory.factories:
            raise NotImplementedError
        return ManifoldShapeFactory.factories[manifold].create(shape, transpose, p_manifold)
...
class StiefelLikeFactory(ManifoldShapeFactory):
    ...
    """
    def create(self, shape, transpose=False, p_manifold=None):
        if len(shape) == 3:
            k, h, w = shape
        elif len(shape) == 2:
            k = 1
        ...
        if h > w:
            to_transpose = False
            to_return = Parameter(manifold=self.manifold(h, w, k=k), p_manifold=p_manifold)
        elif h < w:
            to_transpose = True
            to_return = Parameter(manifold=self.manifold(w, h, k=k), p_manifold=p_manifold)
        elif h == w:
...
class SquareManifoldFactory(ManifoldShapeFactory):
    ...
    def create(self, shape, transpose=False, p_manifold=None):
        ...
        return transpose, Parameter(manifold=self.manifold(n=n, k=k), p_manifold=p_manifold)
...
class EuclideanManifoldFactory(ManifoldShapeFactory):
    ...
    def create(self, shape, transpose=False, p_manifold=None):
        if len(shape) == 0:
            ...
            return transpose, Parameter(manifold=self.manifold(*shape), p_manifold=p_manifold)
...
class DSManifoldFactory(ManifoldShapeFactory):
    ...
    def create(self, shape, transpose=False, p_manifold=None):
        ...
        return transpose, Parameter(manifold=self.manifold(n=n, m=m, k=k), p_manifold=p_manifold)

class BirkhoffManifoldFactory(ManifoldShapeFactory):
    """
    Manifold factory for DoublyStochastic manifold
    """
    def create(self, shape, transpose=False, p_manifold=None):
...
class UpperHalfManifoldFactory(ManifoldShapeFactory):
    """
    Manifold factory for DoublyStochastic manifold
    """
    def create(self, shape, transpose=False, p_manifold=None):
...
BirkhoffManifoldFactory(BirkhoffPolytope)
